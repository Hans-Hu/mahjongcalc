...
const MPSZ = ['m', 'p', 's', 'z']
const KAZE = [undefined, 'East', 'South' ,'West', 'North', 'White', 'Green', 'Red']
const ceil10 = (num)=>{
...
class Riichi {
    ...
    calcFu() { 
        let fu = 0
        if (this.tmpResult.yaku['Chiitoi']) {
            fu = 25
    ...
    calcTen() {
        this.tmpResult.name = ''
        let base
        this.tmpResult.text = `(${KAZE[this.bakaze]} round / `
        this.tmpResult.text += KAZE[this.jikaze] + ' seat) '
        this.tmpResult.text += this.isTsumo ? 'Tsumo' : 'Ron'
        if (this.tmpResult.yakuman) {
            ...
            this.tmpResult.text += ' ' + this.tmpResult.fu + ' Fu / ' + this.tmpResult.han + ' Han'
            if (base > 2000) {
        ...
        for (let k in YAKU) {
            ...
            if (v.check(this)) {
                if (v.yakuman) {
                    ...
                    this.tmpResult.yaku[k] = n > 1 ? '2x Yakuman' : 'Yakuman'
                } else {
                    let n = v.han
